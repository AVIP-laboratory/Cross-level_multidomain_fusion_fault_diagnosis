...
if job == 1:
    ...
    Dense_layer = tf.keras.layers.concatenate([T_C1_layer, T_C2_layer, T_C3_layer, T_V_layer,
                                               FFT_C1_layer, FFT_C2_layer, FFT_C3_layer, FFT_V_layer,
                                               MFCC_C1_layer, MFCC_C2_layer, MFCC_C3_layer, MFCC_V_layer])
    output = Dense(5, activation='softmax')(Dense_layer)

    # Define the model
    model = tf.keras.Model(inputs=[Time_V_Input, Time_C1_Input, Time_C2_Input, Time_C3_Input,
                                   FFT_V_Input, FFT_C1_Input, FFT_C2_Input, FFT_C3_Input,
                                   MFCC_V_Input, MFCC_C1_Input, MFCC_C2_Input, MFCC_C3_Input], outputs=output)
    model.summary()
...
if job == 2:
    ...
    model = load_model(f'{Result_path}/{Model_name}.h5')
    ...
    score = model.evaluate([Test_time_data_V, Test_time_data_C1, Test_time_data_C2, Test_time_data_C3,
                            Test_FFT_data_V, Test_FFT_data_C1, Test_FFT_data_C2, Test_FFT_data_C3,
                            Test_MFCC_data_V, Test_MFCC_data_C1, Test_MFCC_data_C2, Test_MFCC_data_C3],
                            Y_test, verbose=0)
    print(f"Test loss: {score[0]}\n Test accuracy: {score[1]}")

    # Save test results to CSV
    a1 = pd.DataFrame(score)
    a1.to_csv(f'{Result_path}/{Model_name}_Noisetest_result.csv',
    ...
    plt.savefig(f'{Result_path}/{Model_name}_ConfusionMatrix_NoiseX.tiff')
    ...
    for noise in noiselist:
        ...
        y_pred = model.predict([Test_time_data_V, Test_time_data_C1, Test_time_data_C2, Test_time_data_C3,
                                Test_FFT_data_V, Test_FFT_data_C1, Test_FFT_data_C2, Test_FFT_data_C3,
                                Test_MFCC_data_V, Test_MFCC_data_C1, Test_MFCC_data_C2, Test_MFCC_data_C3])
        np.save(f'{Result_path}/{Model_name}_predict(5)_Noise{noise}_for t-sne.npy', y_pred)
        YY_test = np.argmax(Y_test_categorical, axis=1)
        YY_pred = np.argmax(y_pred, axis=1)
        ...
        disp = ConfusionMatrixDisplay(confusion_matrix=report, display_labels=label)
        disp.plot(cmap='Blues')
        plt.savefig(f'{Result_path}/{Model_name}_ConfusionMatrix_Noise{noise}.tiff')
